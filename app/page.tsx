    ...
    <main className="min-h-screen overflow-x-hidden bg-[#070817] text-white">
      ...
      <div className="mx-auto grid max-w-[1500px] gap-5 px-4 py-5 sm:px-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:px-8">
        ...
        <aside className="grid content-start gap-5">
          ...
          <section id="leaderboard" className="rounded-[24px] border border-white/10 bg-white/[.045] p-5"><div className="mb-4 flex items-center justify-between"><div><p className="text-xs font-black uppercase tracking-[.18em] text-amber-300">Weekly league</p><h2 className="mt-1 text-xl font-black">Leaderboard</h2></div><Trophy className="text-amber-300" /></div><ol className="space-y-2">{leaderboard.map((player) => <li key={player.rank} className={`grid grid-cols-[28px_38px_1fr_auto] items-center gap-2 rounded-xl px-2 py-2.5 ${player.you ? "border border-lime-300/25 bg-lime-300/10" : ""}`}><span className="text-center text-sm font-black text-white/40">{player.rank}</span><span className={`grid size-9 place-items-center rounded-full font-black ${player.you ? "bg-lime-300 text-slate-950" : "bg-white/10"}`}>{player.avatar}</span><span className="min-w-0"><span className="block truncate text-sm font-extrabold">{player.name}</span><span className="block text-xs text-white/35">🔥 {player.streak} days</span></span><span className="text-sm font-black">{player.score.toLocaleString()}</span></li>)}</ol></section>
        </aside>
      </div>
    </main>
  );
