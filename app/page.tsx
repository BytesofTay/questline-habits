...
import { addQuest as addQuestToProgress, completeQuest as completeQuestInProgress, initialProgress, loadProgress, saveProgress } from "@/lib/questline-state.mjs";
...
export default function Home() {
  ...
  const { quests, coins, xp } = progress;
  ...
  function completeVisibleQuest(id: number) {
    setProgress((current) => completeQuestInProgress(current, id));
  }

  function addQuest() {
    const title = newQuest.trim();
    if (!title) return;
    setProgress((current) => addQuestToProgress(current, title));
    setNewQuest("");
    setDialogOpen(false);
  }
  ...
  function useEnergyStrike() {
    ...
    if (nextHp === 0) {
      setBattleState("won");
      setProgress((current) => ({ ...current, coins: current.coins + 120, xp: Math.min(100, current.xp + 10) }));
    }
  }
  ...
  useEffect(() => {
    if (progressLoaded) saveProgress(browserStorage(), progress);
  ...
  useEffect(() => {
    ...
    void Promise.resolve(context.registerTool({
      ...
      execute(input) {
        ...
        setProgress((current) => addQuestToProgress(current, title, quest.id));
    ...
    <main className="battle-royale-ui min-h-screen overflow-x-hidden bg-[#071b36] text-white">
        ...
        <div className="mx-auto max-w-[1500px] px-4 pb-3 sm:px-6 lg:px-8">
          <section id="leaderboard" aria-label="Weekly leaderboard" className="leaderboard-glow flex h-[66px] w-full items-center overflow-hidden rounded-full border border-amber-300/25 bg-[#11142b]/95 shadow-[0_14px_35px_rgba(0,0,0,.38)]">
            <div className="flex h-full shrink-0 items-center gap-2 border-r border-white/10 bg-gradient-to-r from-amber-300/15 to-transparent px-4 sm:px-5">
              ...
              <span className="hidden sm:block"><span className="block text-[11px] font-black uppercase tracking-[.18em] text-amber-300">Sample league</span><span className="block text-sm font-black">Illustrative ranks</span></span>
            </div>
      ...
      <div className="mx-auto grid max-w-[1500px] gap-5 px-4 py-5 sm:px-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:px-8">
          ...
          <div className="game-panel arena-grid relative min-h-[390px] overflow-hidden rounded-[28px] border border-white/10 bg-[#0c2345] p-5 sm:p-8">
            ...
            <div className="relative z-10 grid h-full items-center gap-6 md:grid-cols-[1fr_340px]">
              <div className="max-w-xl">
                ...
                <p className="mt-5 max-w-md text-base leading-7 text-white/60">Queue your real-life quests, charge your champion, and preview how shared goals could power a friendly competition.</p>
        ...
        <aside className="grid content-start gap-5">
          ...
          <section id="squad" className="game-panel rounded-[24px] border border-cyan-300/15 bg-cyan-300/[.055] p-5"><div className="flex items-start justify-between"><div><p className="text-xs font-black uppercase tracking-[.18em] text-cyan-300">Squad finder prototype</p><h2 className="mt-1 text-xl font-black">{matched ? "Sample match" : "Preview a possible squad"}</h2></div><Swords className="text-cyan-300" /></div><p className="mt-3 text-sm leading-6 text-white/55">This demo shows a sample match card. Real matching needs accounts, consent, and a backend; no players are contacted.</p>{matched ? <div className="mt-4 rounded-2xl border border-cyan-300/20 bg-black/20 p-4"><p className="font-black">Sample: Momentum Crew</p><p className="mt-1 text-sm text-white/50">4 players · Fitness + Learning · Friendly competitive</p></div> : null}<Button className="mt-4 w-full rounded-xl bg-cyan-300 font-black text-slate-950 hover:bg-cyan-200" onClick={() => setMatched(true)}><Users /> {matched ? "View sample match" : "Preview sample match"}</Button></section>
        </aside>
      </div>
